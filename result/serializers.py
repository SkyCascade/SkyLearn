...
class StudentGradeSerializer(serializers.ModelSerializer):
    ...
    student_id = serializers.CharField(
        source="student.student.username", read_only=True
    )
...
class LecturerGrade1stModuleSerializer(StudentGradeSerializer):
    ...
    def validate(self, data):
        attendance = data.get(
            "attendance", self.instance.attendance if self.instance else 0
        )
        activities = data.get(
            "activities", self.instance.activities if self.instance else 0
        )
...
class BulkGradeUpdateSerializer(serializers.Serializer):
    ...
    activities = serializers.DecimalField(
        max_digits=5, decimal_places=2, required=False
    )
...
class BulkGradesUpdateSerializer(serializers.Serializer):
    ...
    grade_type = serializers.ChoiceField(
        choices=["1st_module", "2nd_module", "semester"]
    )
