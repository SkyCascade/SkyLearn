...
class LessonTimeViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_context(self):
        ...
        context["admin"] = (
            ...
            else getattr(self.request.user, "admin", None)
        )
        return context
...
class AttendanceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Преподаватели видят посещаемость на своих занятиях
            ...
            for allocation in allocations:
                ...
                if allocation.group:
                    queries |= Q(
                        shcedule__course__in=course_ids,
                        shcedule__group=allocation.group,
                    )
                else:
    ...
    def bulk_update(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Занятие не найдено"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if request.user.is_lecturer and not request.user.is_staff:
            ...
            has_permission = CourseAllocation.objects.filter(
                lecturer=request.user, courses=schedule.course, group=schedule.group
            ).exists()
